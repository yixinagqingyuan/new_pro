        ...
        <div class="rating">
          ...
          <div class="rating-wrapper">
            <ul><!-- ctrl + shift +f  format-->
              <li class="rating-item border-1px" v-for="rating in filterRatings" :key="rating.id">
                <div class="user">
  ...
  export default {
    ...
    methods: {
      show(isShow) {
        ...
        if (isShow) {
          //nextTick获取跟新后的DOM，$refs获取页面上的页面上的dom由于nexttick是放在methods中，已经是dom加载完成的钩子，说以可以去掉
          Vue.nextTick(() => {
            if (!this.scroll) {
