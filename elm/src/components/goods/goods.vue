  ...
  export default {
    ...
    methods: {
      ...
      _initTops() {
        ...
        //数组的方法被当前的dom对象强行调用，转化成数组类型，并且使用forEach遍历得到每个数组中的内容注意，forEach调用写上形参，回调函数内部自动调用接收
        [].slice.call(lis).forEach(li => {
          //console.log(li);
          //clientHeight：窗口元素大小，尽量不要  使用scrollHeight
          top += li.clientHeight;
          //console.log(top);
           tops.push(top);
        });
      ...
      //父组件给子组件传值除了可以传变量中的值，还能传一个方法，子组件使用父组件的值和调用父组件方法，但是不能更改
      updateFoodCount(food, isAdd, event) {
        ...
        if (isAdd) { // 增加
          ...
          //父组件想要调用子组件的方法，需要用$refs方法把子组件中的ref自定义属性拿到拿到子组件的方法，然后调用
